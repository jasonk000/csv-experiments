...
unsigned char
_csv_next_char(unsigned char *buffer, size_t pos)
{
  return buffer[pos];
}

size_t
csv_parse(struct csv_parser *p, const void *s, const void *avx, size_t len, void (*cb1)(void *, size_t, void *), void (*cb2)(int c, void *), void *data)
{
  unsigned const char *us = s;  /* Access input data as array of unsigned char */
  unsigned const char *uavx = avx; /* Access input data as array of unsigned char */
  ...
  while (pos < len) {
    ...
    /* be careful to check we have at least 8 slots in case an avx parse succeeds */
    if ((entry_pos + 8) >= ((p->options & CSV_APPEND_NULL) ? p->entry_size - 1 : p->entry_size) ) {
      if (csv_increase_buffer(p) != 0) {
        p->quoted = quoted, p->pstate = pstate, p->spaces = spaces, p->entry_pos = entry_pos;
        return pos;
      }
    ...
    // check full avx byte, and just be sure we won't copy any extra 0's from 
    // the end of the buffer (so don't check past end of buff)
    if (uavx != 0 && (pos % 8 == 0) && (uavx[pos/8] == 0) && (pos < (len - 8)) && pstate == FIELD_BEGUN) {
      SUBMIT_CHAR(p, us[pos++]);
      SUBMIT_CHAR(p, us[pos++]);
      SUBMIT_CHAR(p, us[pos++]);
      SUBMIT_CHAR(p, us[pos++]);
      SUBMIT_CHAR(p, us[pos++]);
    ...
    if (c != CSV_COMMA && c != CSV_CR && c != CSV_LF && c != CSV_SPACE && c != CSV_QUOTE && pstate == FIELD_BEGUN) {
      SUBMIT_CHAR(p, c);
      continue;
    }
